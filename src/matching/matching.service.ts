...
export class MatchingService {
  calculateScore(
    jobDetail: JobExtractionResult,
    profile: ListUserProfileDto,
  ): number {
    const { modality, salary, skills, experience_years, is_job, location } =
      jobDetail;
    const {
      modality: profileModality,
      salaryMin,
      salaryMax,
      experience_years: profileExperienceYears,
    ...
    if (modality && profileModality) {
      const modalityScore = this.calculateModalityScore(
        modality,
        profileModality,
      );
    ...
    // verify salary
    if (salary && salaryMin) {
      scoreValues.salary.value = this.calculateSalaryScore(
        salary,
        salaryMin,
        salaryMax,
      );
    ...
    if (experience_years && profileExperienceYears) {
      scoreValues.experience.value = this.calculateExperienceScore(
        experience_years,
    ...
    if (skills && profile.skills) {
      scoreValues.skills.value = this.calculateSkillsScore(
        skills,
        profile.skills,
      );
    }
  ...
  private readonly calculateModalityScore = (
    modality: string,
    profileModality: string[],
  ): number => {
  ...
  private readonly calculateSalaryScore = (
    ...
    const jobSalary = min && max ? (min + max) / 2 : min || max!;
  ...
  private readonly calculateExperienceScore = (
    ...
    // proporcional
    return Math.max(0, Math.min(candidateYears / requiredYears, 1));
  };

  private readonly calculateFinalScore = (scoreValues: IMatch): number => {
    const finalScore = Object.values(scoreValues).reduce((acc, item) => {
  ...
  private readonly calculateSkillsScore = (
    ...
    for (const requiredSkill of requiredSkills) {
      const found = candidateSkills.some((candidateSkill) =>
        this.skillMatches(candidateSkill, requiredSkill),
      );
  ...
  private readonly skillMatches = (
    ...
    const fuse = new Fuse([normalizedRequired], {
      includeScore: true,
      threshold: 0.3,
    });

    const result = fuse.search(normalizedCandidate);
  ...
  private readonly normalizeSkill = (skill: string): string => {
